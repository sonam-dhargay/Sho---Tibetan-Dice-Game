...
const App: React.FC = () => {
  ...
  const [opponentSelection, setOpponentSelection] = useState<{ color: string, avatar: string } | null>(null);
  const [selectionConflictMsg, setSelectionConflictMsg] = useState(false);
  ...
  const handleColorSelect = (hex: string) => {
    triggerHaptic(10);
    if (opponentSelection && opponentSelection.color === hex) {
      triggerHaptic([100, 50, 100]);
      SFX.playBlocked();
      setSelectionConflictMsg(true);
      setTimeout(() => setSelectionConflictMsg(false), 3000);
      return;
    }
    setSelectedColor(hex);
    if (onlineLobbyStatus === 'CONNECTED') {
      broadcastPacket({ type: 'SELECTION_UPDATE', payload: { color: hex, avatar: selectedAvatar } });
      setPlayers(prev => {
        const next = [...prev];
        const localIdx = gameModeRef.current === GameMode.ONLINE_GUEST ? 1 : 0;
        next[localIdx] = { ...next[localIdx], colorHex: hex };
        return next;
      });
    }
  };
  ...
  const handleAvatarSelect = (url: string) => {
    triggerHaptic(10);
    if (opponentSelection && opponentSelection.avatar === url) {
      triggerHaptic([100, 50, 100]);
      SFX.playBlocked();
      setSelectionConflictMsg(true);
      setTimeout(() => setSelectionConflictMsg(false), 3000);
    ...
    if (onlineLobbyStatus === 'CONNECTED') {
      broadcastPacket({ type: 'SELECTION_UPDATE', payload: { color: selectedColor, avatar: url } });
      setPlayers(prev => {
        const next = [...prev];
        const localIdx = gameModeRef.current === GameMode.ONLINE_GUEST ? 1 : 0;
        next[localIdx] = { ...next[localIdx], avatarUrl: url };
        return next;
      });
    }
  };
  ...
  useEffect(() => {
    if (gameMode === GameMode.ONLINE_HOST && phase !== GamePhase.SETUP) {
  ...
  const initializeGame = useCallback((p1Config: { name: string, color: string, avatar?: string }, p2Config: { name: string, color: string, avatar?: string }, isTutorial = false, initialPhase = GamePhase.ROLLING) => {
    ...
    setPlayers(initialPlayers); setTurnIndex(0); setPhase(initialPhase); setLastRoll(null); setIsRolling(false); setPendingMoveValues([]); setPaRaCount(0); setExtraRolls(0); setIsOpeningPaRa(false); setLastMove(null); setTutorialStep(isTutorial ? 1 : 0); setSelectedSourceIndex(null); setWinner(null);
    addLog("Match initialized.", 'info');
  }, [addLog]);
  ...
  const handleNetworkPacket = useCallback((packet: NetworkPacket) => {
    switch (packet.type) {
      ...
      case 'SELECTION_UPDATE':
        setOpponentSelection({ color: packet.payload.color, avatar: packet.payload.avatar });
        setPlayers(prev => {
            const next = [...prev];
            const oppIdx = gameModeRef.current === GameMode.ONLINE_HOST ? 1 : 0;
            next[oppIdx] = { ...next[oppIdx], colorHex: packet.payload.color, avatarUrl: packet.payload.avatar };
            return next;
        });
        break;
      case 'JOIN_INFO':
        if (gameModeRef.current === GameMode.ONLINE_HOST) {
            ...
            setOpponentSelection({ color: guestColor, avatar: guestAvatar });
            setPlayers(prev => {
                const next = [...prev];
            ...
            setOpponentSelection({ color: hostColor, avatar: hostAvatar });
            setPlayers(prev => {
                const next = [...prev];
  ...
  const startOnlineHost = () => {
    ...
    newPeer.on('connection', (conn) => {
      conn.on('open', () => {
        ...
        initializeGame({ name: getSafePlayerName(), color: selectedColor, avatar: selectedAvatar }, { name: 'Opponent...', color: '#3b82f6' }, false, GamePhase.SETUP);
        addLog("Guest connected!", 'info');
      });
  ...
  const joinOnlineMatch = (code: string) => {
    ...
    newPeer.on('open', () => {
      ...
      conn.on('open', () => {
        ...
        initializeGame({ name: 'Host...', color: '#ef4444' }, { name: getSafePlayerName(), color: selectedColor, avatar: selectedAvatar }, false, GamePhase.SETUP);
  ...
  const handleStartMatch = () => {
    if (gameMode !== GameMode.ONLINE_HOST) return;
    triggerHaptic(20);
    broadcastPacket({ type: 'START_GAME' });
    setPhase(GamePhase.ROLLING);
    addLog("Match started!", "info");
  };

  const showRoomLobby = (onlineLobbyStatus === 'WAITING' || (gameMode && phase === GamePhase.SETUP));
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-stone-900 text-stone-100' : 'bg-stone-100 text-stone-900'} flex flex-col md:flex-row fixed inset-0 font-sans mobile-landscape-row transition-colors duration-500`}>
        ...
        <style dangerouslySetInnerHTML={{__html: `
          ...
          @keyframes shake-tiny { 0%, 100% { transform: translate(0,0); } 25% { transform: translate(-2px,0); } 75% { transform: translate(2px,0); } }
          .animate-shake-tiny { animation: shake-tiny 0.1s linear infinite; }
        `}} />
        ...
        {phase === GamePhase.GAME_OVER && winner && (
          <div className="fixed inset-0 z-[200]">
            <VictoryOverlay winner={winner} isDarkMode={isDarkMode} onRestart={resetToLobby} />
          </div>
        )}
        
        {isAuthModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
            <div className={`${isDarkMode ? 'bg-stone-900 border-amber-600/50' : 'bg-stone-50 border-amber-800/20'} border-2 p-8 rounded-[3rem] w-full max-w-sm shadow-2xl relative`}>
        ...
        {isLoginGateOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in zoom-in duration-300">
            <div className={`${isDarkMode ? 'bg-stone-900 border-amber-600/30' : 'bg-white border-stone-300'} border-2 p-8 rounded-[3rem] w-full max-sm shadow-2xl text-center`}>
              <h2 className="text-3xl font-cinzel text-amber-500 mb-6 font-bold tracking-widest">{T.auth.gateTitle.en}</h2>
              <div className="flex flex-col gap-4">
                <button onClick={() => { triggerHaptic(15); setAuthMode('LOGIN'); setIsAuthModalOpen(true); setIsLoginGateOpen(false); }} className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all active:scale-95 flex flex-col items-center">
                ...
                <button onClick={() => { triggerHaptic(15); setAuthMode('SIGNUP'); setIsAuthModalOpen(true); setIsLoginGateOpen(false); }} className={`w-full py-4 ${isDarkMode ? 'bg-stone-800 text-stone-200' : 'bg-stone-100 text-stone-700'} border font-bold rounded-2xl transition-all active:scale-95 flex flex-col items-center`}>
                ...
                <button onClick={() => setIsLoginGateOpen(false)} className="text-stone-500 uppercase text-xs mt-4">Close</button>
              </div>
            </div>
          </div>
        )}

        {!gameMode && !showRoomLobby && (
          <div className={`fixed inset-0 z-50 ${isDarkMode ? 'bg-stone-950 text-amber-500' : 'bg-stone-50 text-amber-800'} overflow-y-auto flex flex-col items-center justify-between p-6 transition-colors duration-500`}>
              <div className="flex-grow flex flex-col items-center justify-center w-full max-w-md gap-6">
                  <img src={EXTERNAL_LOGO_URL} alt="Sho Logo" className="w-48 h-48 object-contain mb-4 drop-shadow-2xl" />
                  <h1 className="flex items-center gap-6 font-cinzel">
                      <span className="text-6xl">{T.lobby.title.bo}</span>
                      <span className="text-4xl tracking-widest">{T.lobby.title.en}</span>
                  </h1>
                  <div className="w-full flex flex-col gap-4 mt-8">
                      <button onClick={() => { triggerHaptic(20); setGameMode(GameMode.LOCAL); gameModeRef.current = GameMode.LOCAL; initializeGame({name: getSafePlayerName(), color: selectedColor, avatar: selectedAvatar}, {name: 'Player 2', color: '#999', avatar: AVATAR_LIST[1]}); }} className="w-full py-4 bg-stone-900 border border-stone-800 text-amber-500 rounded-2xl font-bold uppercase tracking-widest flex flex-col items-center">
                          <span>{T.lobby.modeLocal.en}</span>
                          <span className="text-sm font-serif">{T.lobby.modeLocal.bo}</span>
                      </button>
                      <button onClick={handleOnlineClick} className="w-full py-4 bg-amber-600 text-white rounded-2xl font-bold uppercase tracking-widest flex flex-col items-center">
                          <span>{T.lobby.modeMulti.en}</span>
                          <span className="text-sm font-serif">{T.lobby.modeMulti.bo}</span>
                      </button>
                  </div>
              </div>
          </div>
        ...
        {showRoomLobby && (
          <div className={`fixed inset-0 z-[150] ${isDarkMode ? 'bg-stone-950 text-amber-500' : 'bg-stone-50 text-amber-800'} overflow-y-auto flex flex-col items-center justify-center p-6 gap-8`}>
            {selectionConflictMsg && (
              <div className="absolute top-10 left-1/2 -translate-x-1/2 w-full max-w-xs animate-in slide-in-from-top-2 fade-in duration-300 z-50">
                <div className="bg-red-600 text-white p-4 rounded-2xl shadow-xl text-center border-2 border-red-400">
                  <p className="text-[10px] font-bold uppercase tracking-widest">{T.lobby.conflictTitle.en}</p>
                  <p className="text-sm font-serif mt-1">{T.lobby.conflictDesc.bo}</p>
                </div>
              </div>
            )}

            <div className="w-full max-w-md bg-stone-900/40 p-8 rounded-[3rem] border border-stone-800/20 backdrop-blur-2xl shadow-2xl">
              <h2 className="text-2xl font-cinzel text-center text-amber-500 mb-8 uppercase tracking-widest">
                {T.lobby.roomLobbyTitle.en} <div className="font-serif text-lg mt-1">{T.lobby.roomLobbyTitle.bo}</div>
              </h2>

              <div className="mb-8">
                <label className="text-stone-500 text-[10px] uppercase block mb-4 tracking-widest font-bold text-center">Choose Avatar འདྲ་པར་དོམ།</label>
                <div className="grid grid-cols-4 gap-3">
                  {AVATAR_LIST.map((url, idx) => {
                    const isTaken = opponentSelection?.avatar === url;
                    return (
                      <button key={idx} onClick={() => handleAvatarSelect(url)} className={`aspect-square rounded-2xl overflow-hidden border-2 transition-all ${selectedAvatar === url ? 'border-amber-500 scale-110 shadow-lg' : 'border-transparent opacity-60'} ${isTaken ? 'grayscale opacity-20 cursor-not-allowed' : ''}`}>
                        <img src={url} alt={`Avatar ${idx}`} className="w-full h-full object-cover" />
                        {isTaken && <div className="absolute inset-0 flex items-center justify-center bg-red-900/30 text-white font-bold text-[8px] uppercase">Taken</div>}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="mb-8">
                <label className="text-stone-500 text-[10px] uppercase block mb-4 tracking-widest font-bold text-center">Choose Color ཚོས་གཞི་དོམ།</label>
                <div className="flex justify-between gap-2">
                  {COLOR_PALETTE.map((c) => {
                    const isTaken = opponentSelection?.color === c.hex;
                    return (
                      <button key={c.hex} onClick={() => handleColorSelect(c.hex)} className={`w-10 h-10 rounded-xl transition-all rotate-45 ${selectedColor === c.hex ? 'border-2 border-white scale-110' : 'opacity-40'} ${isTaken ? 'grayscale opacity-10 cursor-not-allowed relative' : ''}`} style={{ backgroundColor: c.hex }}>
                        {isTaken && <div className="absolute inset-0 flex items-center justify-center -rotate-45 text-white font-bold text-[10px]">X</div>}
                      </button>
                    );
                  })}
                </div>
              ...
              {onlineLobbyStatus === 'WAITING' ? (
                  <div className="flex flex-col gap-6">
                    <div className="bg-stone-950 p-6 rounded-3xl border border-stone-800 text-center">
                        <p className="text-stone-500 text-xs uppercase mb-2">Room Code ཁང་བའི་ཨང་གྲངས།</p>
                        <p className="text-amber-500 text-4xl font-mono font-bold tracking-widest">{myPeerId || '---'}</p>
                    </div>
                    {myPeerId ? (
                        <div className="flex flex-col gap-2">
                          <button onClick={() => { triggerHaptic(10); navigator.clipboard.writeText(myPeerId); addLog("Code copied!", "info"); }} className="text-stone-500 uppercase text-[10px] tracking-widest">Copy Code 📋</button>
                        </div>
                    ) : (
                      <div className="grid grid-cols-1 gap-4">
                        <div className="bg-stone-800/50 p-6 rounded-3xl border border-stone-700 text-center">
                           <input type="text" value={joinCodeInput} onChange={(e) => setJoinCodeInput(e.target.value.toUpperCase())} placeholder="ENTER CODE" className="w-full bg-transparent border-b-2 border-stone-600 text-amber-500 font-mono text-2xl text-center mb-4 focus:border-amber-500 outline-none" />
                           <div className="grid grid-cols-2 gap-3">
                             <button onClick={() => startOnlineHost()} className="py-3 bg-stone-900 border border-stone-700 text-stone-300 rounded-xl font-bold uppercase text-[10px] tracking-widest">HOST MATCH</button>
                             <button onClick={() => joinOnlineMatch(joinCodeInput)} className="py-3 bg-amber-600 text-white rounded-xl font-bold uppercase text-[10px] tracking-widest">JOIN MATCH</button>
                           </div>
                        </div>
                      </div>
                    )}
                ...
                <div className="flex flex-col gap-4">
                    {gameMode === GameMode.ONLINE_HOST ? (
                      <button onClick={handleStartMatch} className="w-full py-5 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-2xl shadow-xl animate-gold-pulse flex flex-col items-center">
                        <span className="uppercase tracking-[0.2em]">{T.lobby.startGame.en}</span>
                        <span className="font-serif text-sm mt-1">{T.lobby.startGame.bo}</span>
                      </button>
                    ) : (
                      <div className="p-6 bg-stone-800/40 rounded-2xl border border-amber-600/30 text-center">
                        <p className="text-amber-500 text-sm font-cinzel animate-pulse">{T.lobby.waiting.en}</p>
                        <p className="text-amber-600 font-serif mt-2">{T.lobby.waiting.bo}</p>
                      </div>
                    )}
                </div>
              )}
              
              <button onClick={resetToLobby} className="w-full mt-6 text-stone-500 uppercase text-xs">Back to Main</button>
            </div>
          </div>
        )}

        {gameMode && phase !== GamePhase.SETUP && (
            <>
                <div className={`w-full md:w-1/4 flex flex-col border-r ${isDarkMode ? 'border-stone-800 bg-stone-950' : 'border-stone-200 bg-white'} z-20 h-full overflow-hidden transition-colors duration-500`}>
                    <div className="p-4 flex flex-col gap-3">
                        <header className="flex justify-between items-center border-b pb-4">
                            <div className="flex items-center gap-2 cursor-pointer" onClick={resetToLobby}>
                                <img src={EXTERNAL_LOGO_URL} alt="Logo" className="w-8 h-8 object-contain" />
                                <h1 className="font-cinzel text-sm">Sho</h1>
                            </div>
                            <button onClick={() => setShowMenu(true)} className="p-2 border rounded-xl">☰</button>
                        </header>
                        <div className="grid grid-cols-2 gap-2 mt-4">
                            {players.map((p, i) => (
                                <div key={p.id} className={`p-3 rounded-xl border ${turnIndex === i ? 'border-amber-500 bg-amber-900/10' : 'opacity-60'}`}>
                                    <div className="flex items-center gap-2 mb-2">
                                        <img src={p.avatarUrl || AVATAR_LIST[0]} className="w-6 h-6 rounded-full border border-white/20" />
                                        <span className="font-bold text-[10px] truncate" style={{ color: p.colorHex }}>{p.name}</span>
                                    </div>
                                    <div className="flex justify-between text-xs font-bold">
                                        <span>In: {p.coinsInHand}</span>
                                        <span className="text-amber-500">Out: {p.coinsFinished}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="px-4 pb-4 mt-auto">
                        <DiceArea currentRoll={lastRoll} onRoll={performRoll} canRoll={phase === GamePhase.ROLLING && isLocalTurn} pendingValues={pendingMoveValues} waitingForPaRa={paRaCount > 0} paRaCount={paRaCount} extraRolls={extraRolls} flexiblePool={null} />
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <button onClick={handleFromHandClick} className={`p-4 rounded-xl border-2 font-bold uppercase text-[10px] ${shouldHighlightHand ? 'border-amber-500 bg-amber-900/20' : 'border-stone-800'}`}>Hand ({players[turnIndex].coinsInHand})</button>
                          {phase === GamePhase.MOVING && isLocalTurn && pendingMoveValues.length > 0 && (
                            <button onClick={() => handleSkipTurn()} className="bg-stone-800 text-stone-400 p-4 rounded-xl text-[10px] uppercase font-bold">Skip</button>
                          )}
                        </div>
                    </div>
                </div>
                <div className={`flex-grow relative ${isDarkMode ? 'bg-stone-900' : 'bg-stone-100'} flex items-center justify-center overflow-hidden transition-colors duration-500`} ref={boardContainerRef}>
